...
def is_position_valid(route, pos, N, M):
    """
    Kiểm tra vị trí chèn (giữa route[pos] và route[pos+1]) có nằm giữa
    cặp pickup–dropoff của một hành khách đã có hay không.
    Nếu có, không được chèn bất kỳ điểm nào vào đó.
    """
    if pos < 0 or pos + 1 >= len(route):
        return False
    pickup = route[pos]
    # Pickup của passenger nằm trong khoảng 1..N
    if 1 <= pickup <= N:
        dropoff = pickup + N + M
        if route[pos + 1] == dropoff:
            return False
    return True


def find_best_insertion(unassigned_passengers, unassigned_parcels, routes, routes_parcels,
                        capacities, N, M, parcel_weights, dist, route_costs):
    ...
    # Duyệt các hành khách chưa gán
    for p_id in unassigned_passengers:
        pickup = p_id + 1
        dropoff = pickup + N + M
        for r_idx, route in enumerate(routes):
            for pos in range(len(route) - 1):
                if not is_position_valid(route, pos, N, M):
                    continue
                delta = calc_delta_passenger(route, pos, pickup, dropoff, dist)
    ...
    # Duyệt các bưu kiện chưa gán
    for c_id in unassigned_parcels:
        pickup = N + c_id + 1
        dropoff = 2 * N + M + c_id + 1
        weight = parcel_weights[c_id]
        for r_idx, route in enumerate(routes):
            ...
            for pos in range(len(route) - 1):
                ...
                delta = calc_delta_parcel(route, pos, pickup, dropoff, weight,
                                          capacities[r_idx], dist, parcel_list)
